...
# Ensure the columns are of string type before replacing values
df_popselectedvar['pop_initial'] = df_popselectedvar['pop_initial'].astype(str).replace({'Good':'good', 'Fair':'fair', 'Poor':'bad'})
df_popselectedvar['pop_adjusted'] = df_popselectedvar['pop_adjusted'].astype(str).replace({'Good':'good', 'Fair':'fair', 'Bad':'bad'})
df_popselectedvar['pop_country'] = df_popselectedvar['pop_country'].astype(str).replace({
    'Belgium':'BE', 'Estonia':'EE', 'Finland':'FI', 'France':'FR', 
    'Greece':'EL', 'Romania':'RO', 'Serbia':'RS'
})
df_popselectedvar['pop_age'] = df_popselectedvar['pop_age'].astype(str).replace({
    '55-64':'55_64', '65-74':'65_74', '75+':'75_more'
})

print(df_popselectedvar)

# Fetch monetary values from the database
...
# Extract variables from the first row of df_popselectedvar
...
# Filter the initial population data
df_pop_initial = df_pop[(df_pop['country'] == pop_country) & 
                        (df_pop['age_group'] == pop_age) & 
                        (df_pop['group_col'] == pop_initial)]
...
# Filter the adjusted population data
df_pop_adjusted = df_pop[(df_pop['country'] == pop_country) & 
                         (df_pop['age_group'] == pop_age) & 
                         (df_pop['group_col'] == pop_adjusted)]
...
# Function to get population value
